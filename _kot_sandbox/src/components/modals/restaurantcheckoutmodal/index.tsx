...
import { generateKOTHTML, type KOTData } from '../../../utils/kotGenerator';
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      if (
        ...
        // When the backend is submitting in background, close the modal
        // immediately and run KOT + invoice printing asynchronously.
        const submittingInBackground =
          response.message.invoice?.submitting_in_background === true;

        if (submittingInBackground) {
          setIsCreatingInvoice(false);
          onComplete?.(checkoutData);
          onClose();
        }

        // Auto-print logic (runs in background when submittingInBackground)
        const tPrintBlock = performance.now();
        try {
          // --- KOT Printing Start ---
          const tKot = performance.now();
          try {
            // Fetch tags for KOT
              ...
              validItems.map(async (item) => {
                ...
                try {
                  if ((window as any).products?.get) {
                    const product = await (window as any).products.get(
                      item.productCode,
                    );
            ...
            const kotData: KOTData = {
              ...
              orderNumber: invoiceName, // Use Invoice Name as ref
              orderType: orderType,
              items: kotItems,
              timestamp: new Date(),
              tableNo: '', // Not strictly available here unless passed, assume empty or handle if needed
              // orderNotes: '' // Add if available
            };
            ...
            const settings = await window.printerSettings.get();
            const paperSize = settings?.paperSize === '80mm' ? '80mm' : 'A4';

            // Define options based on paper size
            let pdfOptions: any = {};
            if (paperSize === '80mm') {
              // 80mm width, auto height (long enough to fit content without weird breaks if possible)
              // Since Electrons pageSize takes inches/microns, we can try passing the string IF supported,
              // but custom size object is safer for specific thermal dimensions.
              // However, main process now supports passing pageSize directly.
              // If we pass { width: 3.15, height: 100 } (inches), it might work.
              // Or if we use the string "80mm" logic in main? Main just passes it through.
              // Electron expects: "A4", "Letter", etc OR { width: number, height: number } (inches)
              pdfOptions.pageSize = { width: 3.14, height: 11.69 }; // Default A4-ish if needed

              // BUT for 80mm thermal:
              pdfOptions.pageSize = { width: 3.149, height: 40 }; // 40 inches long to simulate roll?
              pdfOptions.margins = { top: 0, bottom: 0, left: 0, right: 0 };
            } else {
              pdfOptions.pageSize = 'A4';
            }
            // disabled client driven kot for now
            //           const kotPdfData = await (window as any).pdf.generateFromHTML(kotHtml, pdfOptions);

            // Convert base64 to Blob
            // //    const cleanBase64 = kotPdfData.data.replace(/\s/g, '');
            ...
            console.error('KOT Printing failed:', kotError);
          }
          console.log(
            `[INVOICE-TIMING] KOT block (tag fetches + print) = ${(performance.now() - tKot).toFixed(1)}ms`,
          );
          // --- KOT Printing End ---
          ...
          // Invoice Printing - Always attempt to print on checkout completion (use configured invoice format like invoice history modal)
          if (settings) {
            try {
        ...
        console.log(
          `[INVOICE-TIMING] handleCreateInvoice TOTAL = ${(performance.now() - invoiceT0).toFixed(1)}ms`,
        );

        // If not already closed (i.e. sync flow), close now after printing
        if (!submittingInBackground) {
          onComplete?.(checkoutData);
          onClose();
        }
