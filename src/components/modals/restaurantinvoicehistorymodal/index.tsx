...
import { buildKotHTML } from '../../../utils/kotPrint';
...
> = ({ onClose }) => {
  ...
  // Server-backed KOT: build HTML via configured format and dispatch via usePrinter
  // so useSeparatePrinters + kotPrinter routing stays consistent with other flows.
  const performPrintKOT = async (invoice: SalesInvoice) => {
    const settings = await window.printerSettings.get();
    const html = await buildKotHTML(
      { sales_invoice: invoice.name },
      {
        format: settings?.kotPrintFormat || undefined,
        source:
    ...
    if (!html) throw new Error('Failed to build KOT');
    await print({ type: 'kot', data: html, title: `KOT - ${invoice.name}` });
  };
