...
import { getKotPrintFormats } from '../../../main/api/kot';
...
const GlobalSettingsModal: React.FC<Props> = ({ onClose, onSave }) => {
  ...
  const [printingSettings, setPrintingSettings] = useState({
    ...
    orderPrintFormat: '',
    kotPrintFormat: '',
    kotPrintFormatSource: 'client_template' as 'client_template' | 'server_print_format',
    pdfPrintScale: 'fit' as 'fit' | 'noscale',
    printFirstPageOnly: true,
    kotPrintEnabled: true,
    askBeforeInvoicePrint: false,
    sessionReportPdfWidth: 'a5' as 'a5' | '80mm' | '58mm',
  });
  ...
  /** Combined list: client KOT templates + server-side KOT print formats.
   *  Stored as entries so the source is known when saving. */
  const [kotFormatEntries, setKotFormatEntries] = useState<
    { value: string; label: string; source: 'client_template' | 'server_print_format' }[]
  >([]);
  ...
  const [sessionReportPdfPrint, setSessionReportPdfPrint] = useState(false);
      ...
      try {
        ...
        if (savedSettings) {
          ...
          setPrintingSettings((prev) => ({
            ...
            kotPrintFormat: savedSettings.kotPrintFormat || '',
            kotPrintFormatSource:
              savedSettings.kotPrintFormatSource === 'server_print_format'
                ? 'server_print_format'
                : 'client_template',
            sessionReportPdfWidth:
              savedSettings.sessionReportPdfWidth === '80mm' || savedSettings.sessionReportPdfWidth === '58mm'
                ? savedSettings.sessionReportPdfWidth
                : 'a5',
            pdfPrintScale:
              savedSettings.pdfPrintScale === 'noscale' ? 'noscale' : 'fit',
            printFirstPageOnly: savedSettings.printFirstPageOnly !== false,
            kotPrintEnabled: savedSettings.kotPrintEnabled !== false,
            askBeforeInvoicePrint: savedSettings.askBeforeInvoicePrint === true,
          }));
        ...
        if (savedSettings) {
          ...
          setSessionReportPdfPrint(savedSettings.sessionReportPdfPrintEnabled || false);
        }
        ...
        try {
          ...
          // KOT formats — unified list (client templates + server print formats)
          try {
            const kotFormats = await getKotPrintFormats();
            const entries: typeof kotFormatEntries = [];
            for (const t of kotFormats.client_templates) {
              entries.push({
                value: t.name,
                label: `${t.name}${t.description ? ` — ${t.description}` : ''} (client)`,
                source: 'client_template',
              });
            }
            for (const s of kotFormats.server_print_formats) {
              entries.push({
                value: s.name,
                label: `${s.name} (server)`,
                source: 'server_print_format',
              });
            }
            setKotFormatEntries(entries);
          } catch (kotErr) {
            console.error('Error fetching KOT print formats:', kotErr);
          }
        } catch (formatErr) {
  ...
  const handleSave = async () => {
    ...
    try {
      await window.printerSettings.save({
        ...
        kotPrintFormat: printingSettings.kotPrintFormat,
        kotPrintFormatSource: printingSettings.kotPrintFormatSource,
        ...
        kotPrintEnabled: printingSettings.kotPrintEnabled,
        askBeforeInvoicePrint: printingSettings.askBeforeInvoicePrint,
        ...
        sessionReportPdfPrintEnabled: sessionReportPdfPrint,
        sessionReportPdfWidth: printingSettings.sessionReportPdfWidth,
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-5 border border-gray-100 rounded-2xl space-y-5 bg-white shadow-sm mb-6">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-2">
                          ...
                          <select
                            value={`${printingSettings.kotPrintFormatSource}:${printingSettings.kotPrintFormat}`}
                            onChange={(e) => {
                              ...
                              if (!raw || raw === ':') {
                                setPrintingSettings((s) => ({
                                  ...s,
                                  kotPrintFormat: '',
                                  kotPrintFormatSource: 'client_template',
                                }));
                                return;
                              }
                              ...
                              const source =
                                sourceRaw === 'server_print_format'
                                  ? 'server_print_format'
                                  : 'client_template';
                              setPrintingSettings((s) => ({
                                ...s,
                                kotPrintFormat: value,
                                kotPrintFormatSource: source,
                              }));
                            }}
                            className="w-full px-4 py-3 bg-gray-50 border-0 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all font-medium"
                          >
                            <option value=":">Default (server)</option>
                            {kotFormatEntries.map((entry) => (
                              <option
                  ...
                  <div className="pt-4 border-t border-gray-100 space-y-4">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-all">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (e.target.checked) setSessionReportPdfPrint(false);
                        }}
                        className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                      />
                      <div>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-all">
                      <input
                        type="checkbox"
                        checked={sessionReportPdfPrint}
                        onChange={(e) => {
                          setSessionReportPdfPrint(e.target.checked);
                          if (e.target.checked) setSessionReportClientPrint(false);
                        }}
                        className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                      />
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Fetch the session report as a PDF from the server and send directly to printer
                        </p>
                      </div>
                    </label>

                    {sessionReportPdfPrint && (
                      <div className="flex gap-4 pl-1">
                        {([
                          { id: 'a5' as const, label: 'A5 / Standard' },
                          { id: '80mm' as const, label: 'Thermal 80mm' },
                          { id: '58mm' as const, label: 'Thermal 58mm' },
                        ]).map((opt) => (
                          <label
                            key={opt.id}
                            className="flex items-center gap-2 cursor-pointer p-2.5 border border-gray-100 rounded-xl hover:bg-gray-50 transition-all flex-1"
                          >
                            <input
                              type="radio"
                              name="sessionReportPdfWidth"
                              checked={printingSettings.sessionReportPdfWidth === opt.id}
                              onChange={() =>
                                setPrintingSettings((s) => ({
                                  ...s,
                                  sessionReportPdfWidth: opt.id,
                                }))
                              }
                              className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                            />
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group p-2 hover:bg-gray-50 rounded-lg transition-all mt-2">
                      <input
                        type="checkbox"
                        checked={printingSettings.kotPrintEnabled}
                        onChange={(e) =>
                          setPrintingSettings((s) => ({
                            ...s,
                            kotPrintEnabled: e.target.checked,
                          }))
                        }
                        className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                      />
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Print a separate Kitchen Order Ticket after invoice/order creation. Disable if KOT is already included in your invoice or order print format.
                        </p>
                      </div>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer group p-2 hover:bg-gray-50 rounded-lg transition-all mt-2">
                      <input
                        type="checkbox"
                        checked={printingSettings.askBeforeInvoicePrint}
                        onChange={(e) =>
                          setPrintingSettings((s) => ({
                            ...s,
                            askBeforeInvoicePrint: e.target.checked,
                          }))
                        }
                        className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                      />
                      <div>
                        <p className="font-semibold text-gray-900">
                          Ask before printing invoice
                        </p>
                        <p className="text-xs text-gray-500">
                          Show a confirmation dialog before printing the invoice after checkout. When disabled, invoices print automatically.
                        </p>
                      </div>
                    </label>
                  </div>
